...
Scripting directives are special annotations prefixed with `@OUTPOST:` that you can embed in your scripts to control the user interface and workflow. They enable you to:
...
@OUTPOST:STEP "Building the application"
npm run build

@OUTPOST:STEP "Running tests"
npm test
```
...
**Usage:**
```sh
@OUTPOST:INPUT "Enter value" "default"
```
...
**Example:**
```sh
@OUTPOST:INPUT "Enter the database host" "localhost"
@OUTPOST:INPUT "Enter the database port" "5432"
@OUTPOST:INPUT "Enter the API key" ""
```
...
**Usage:**
```sh
@OUTPOST:SELECT "Select option" "Option 1" "Option 2" "Option 3"
```
...
**Example:**
```sh
@OUTPOST:SELECT "Select deployment environment" "Development" "Staging" "Production"
@OUTPOST:SELECT "Choose backup type" "Full" "Incremental" "Differential"
@OUTPOST:SELECT "Select database version" "PostgreSQL 12" "PostgreSQL 13" "PostgreSQL 14"
```
...
**Usage:**
```sh
@OUTPOST:CONFIRM "Are you sure you want to continue?"
```

**Example:**
```sh
@OUTPOST:STEP "Preparing to delete database"
@OUTPOST:CONFIRM "Are you sure you want to delete the entire database? This cannot be undone."
...
**Usage:**
```sh
@OUTPOST:INFO "Information message"
```

**Example:**
```sh
@OUTPOST:INFO "Docker is not installed. Installing Docker now..."
curl -fsSL https://get.docker.com -o get-docker.sh
sh get-docker.sh

@OUTPOST:INFO "Configuration will be applied after the service restarts."
...
**Usage:**
```sh
@OUTPOST:SUCCESS "Operation completed successfully!"
```

**Example:**
```sh
@OUTPOST:STEP "Deploying application"
docker pull myapp:latest
docker run -d myapp:latest
@OUTPOST:SUCCESS "Application deployed successfully!"

@OUTPOST:STEP "Running health checks"
curl http://localhost:8080/health
@OUTPOST:SUCCESS "All health checks passed!"
```
...
**Usage:**
```sh
@OUTPOST:WARN "Warning: proceeding with caution"
```

**Example:**
```sh
@OUTPOST:STEP "Updating production configuration"
@OUTPOST:WARN "Warning: You are modifying production configuration. Ensure you have a backup."
cp config.prod.yml config.prod.yml.backup
sed -i 's/old_value/new_value/g' config.prod.yml

@OUTPOST:WARN "Warning: This operation requires manual verification in the admin panel."
```
...
**Usage:**
```sh
@OUTPOST:ERROR "Error occurred"
```

**Example:**
```sh
@OUTPOST:STEP "Verifying prerequisites"
if ! command -v docker &> /dev/null; then
    @OUTPOST:ERROR "Docker is not installed. Please install Docker before proceeding."
    exit 1
fi
@OUTPOST:SUCCESS "All prerequisites verified"

@OUTPOST:STEP "Connecting to database"
if ! psql -h localhost -U user -d mydb -c "SELECT 1" &> /dev/null; then
    @OUTPOST:ERROR "Failed to connect to database. Check your connection parameters."
    exit 1
fi
```
...
**Example:**
```sh
@OUTPOST:STEP "Processing large file"
...
@OUTPOST:SUCCESS "File processed successfully"
```
...
**Usage:**
```sh
@OUTPOST:SUMMARY "Summary Title" "Key 1" "Value 1" "Key 2" "Value 2"
```
...
**Example:**
```sh
@OUTPOST:STEP "Gathering server statistics"
uptime_value=$(uptime)
memory_value=$(free -h | grep Mem | awk '{print $2}')
disk_value=$(df -h / | awk 'NR==2 {print $2}')
cpu_value=$(nproc)

@OUTPOST:SUMMARY "Server Information" \
    "Uptime" "$uptime_value" \
    "Total Memory" "$memory_value" \
    "Total Disk" "$disk_value" \
    "CPU Cores" "$cpu_value"

@OUTPOST:STEP "Backup completed"
@OUTPOST:SUMMARY "Backup Results" \
    "Backup Size" "2.5GB" \
...
Directives provide clear navigation through script execution with `@OUTPOST:STEP` helping users understand where they are in the process.

### 2. **User Engagement**
Interactive directives like `@OUTPOST:INPUT`, `@OUTPOST:SELECT`, and `@OUTPOST:CONFIRM` keep users involved and allow customization.

### 3. **Feedback and Status**
Real-time feedback through `@OUTPOST:INFO`, `@OUTPOST:SUCCESS`, `@OUTPOST:WARN`, and `@OUTPOST:ERROR` keeps users informed about script progress and status.

### 4. **Safety**
The `@OUTPOST:CONFIRM` directive prevents accidental execution of critical operations.

### 5. **Progress Visibility**
The `@OUTPOST:PROGRESS` directive provides visual feedback during long operations, improving perceived performance.

### 6. **Structured Results**
The `@OUTPOST:SUMMARY` directive presents comprehensive results in an organized, easy-to-read format.
...
2. **Provide Defaults** - Always offer sensible defaults in `@OUTPOST:INPUT` to reduce required user interaction.

3. **Confirm Critical Operations** - Use `@OUTPOST:CONFIRM` for operations that modify, delete, or deploy to production.

4. **Inform Users** - Use `@OUTPOST:INFO` to explain what's happening, especially during long operations.

5. **Validate Input** - Use `@OUTPOST:SELECT` instead of free-text input when possible to prevent errors.

6. **Progressive Feedback** - Update `@OUTPOST:PROGRESS` frequently to show activity during long operations.

7. **Clear Error Messages** - Use `@OUTPOST:ERROR` with actionable information about what went wrong and how to fix it.

8. **Summarize Results** - Always end important scripts with `@OUTPOST:SUMMARY` showing key results and completion details.
