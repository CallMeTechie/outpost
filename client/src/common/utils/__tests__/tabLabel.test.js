...
test("a plain session is just the server name", () => {
    assert.strictEqual(buildTabLabel(ssh, {}).text, "pve-01");
});

test("a tmux session carries its session name", () => {
    assert.strictEqual(buildTabLabel(tmux, {}).text, "pve-01 · deploy");
});

test("a scripted session carries its script name", () => {
    assert.strictEqual(buildTabLabel(script, {}).text, "pve-01 · backup.sh");
});

test("tmux wins over the script name when both are there", () => {
    assert.strictEqual(buildTabLabel({ ...tmux, scriptName: "backup.sh" }, {}).text, "pve-01 · deploy");
});
...
test("a session without a server object falls back to the OneDrive name", () => {
    assert.strictEqual(buildTabLabel(onedrive, {}).text, "Firmen-Drive");
});

test("a notes session carries the notes suffix", () => {
    assert.strictEqual(buildTabLabel(notes, {}).text, "pve-01 (Notizen)");
});

test("a custom name replaces the base, keeping the type suffix", () => {
    assert.strictEqual(buildTabLabel(sftp, { name: "Backup" }).text, "Backup (SFTP)");
});
...
test("the number comes after the type suffix", () => {
    assert.strictEqual(buildTabLabel(sftp, { number: 2 }).text, "pve-01 (SFTP) (2)");
});

test("a custom name suppresses the tmux discriminator", () => {
    assert.strictEqual(buildTabLabel(tmux, { name: "Deploy Prod" }).text, "Deploy Prod");
});

test("number 1 shows no suffix", () => {
    assert.strictEqual(buildTabLabel(ssh, { number: 1 }).text, "pve-01");
});

test("a joined session, which carries nothing extra, is just the server name", () => {
    assert.strictEqual(buildTabLabel(joined, {}).text, "pve-01");
});
...
test("the tmux name is sanitised before it reaches the text", () => {
    assert.strictEqual(buildTabLabel({ ...tmux, tmuxSession: "de‮ploy" }, {}).text, "pve-01 · deploy");
});

test("a discriminator that sanitises to nothing leaves no dangling separator", () => {
    assert.strictEqual(buildTabLabel({ ...tmux, tmuxSession: "‮" }, {}).text, "pve-01");
});

// --- tooltip ---

test("the tooltip always carries server and type", () => {
    assert.ok(buildTabLabel(sftp, {}).tooltip.length > 0);
});

test("the tooltip carries the tmux window, which never reaches the text", () => {
    const { text, tooltip } = buildTabLabel({ ...tmux, tmuxWindowId: "@3" }, {});
...
test("fields without a value never appear", () => {
    const { tooltip } = buildTabLabel(ssh, {});
