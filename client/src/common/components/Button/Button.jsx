...
export const Button = ({onClick, text, icon, disabled, type, buttonType, title, dataUiId, ariaInvalid}) => {
    return (
        <button className={"btn" + (type ? " type-" + type : "") + (!text ? " icon-only" : "")} onClick={onClick} disabled={disabled} type={buttonType} data-ui-id={dataUiId} aria-invalid={ariaInvalid || undefined} title={title}>
