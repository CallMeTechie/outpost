...
const parseOutpostUserAgent = (ua) => {
    if (!ua) return null;
    const conn = ua.match(/^OutpostConnector\/([\d.]+)\s*\(([^;]+);\s*([^)]+)\)/);
    if (conn) return { app: 'Outpost Connector', version: conn[1], os: conn[2].trim(), osVersion: conn[3].trim(), icon: 'connector' };
    const mobile = ua.match(/^OutpostMobile\/([\d.]+)\s*\(([^;]+);?\s*([^)]*)\)/);
    if (mobile) return { app: 'Outpost Mobile', version: mobile[1], os: mobile[2].trim().replace(/^\w/, c => c.toUpperCase()), osVersion: mobile[3]?.trim() || '', icon: 'mobile' };
    return null;
};
...
export const Sessions = () => {
    ...
    const getSessionDisplay = (ua) => {
        const n = parseOutpostUserAgent(ua);
