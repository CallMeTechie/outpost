...
if [ -x /usr/local/bin/outpost-engine ]; then
    ...
    MAX_CORES=${OUTPOST_MAX_CORES:-10}
    mkdir -p "$CRASH_DIR"
    ulimit -c unlimited 2>/dev/null || true

    echo "[outpost] Starting server..."
...
EOF
    ...
    preserve_core() {
        ...
        if [ -f "$CRASH_DIR/core" ]; then
            ...
            echo "[outpost] Preserved engine core dump: $core_file"
            ...
            echo "[outpost] Engine (pid $1) exited without a core dump"
            echo "$ts [SUPERVISOR] engine (pid $1) exited unexpectedly; no core dump was produced" >> "$ENGINE_LOG"
        fi
    }

    start_engine() {
        echo "[outpost] Starting local engine..."
        cd "$CRASH_DIR" && /usr/local/bin/outpost-engine 2>> "$ENGINE_LOG" &
        ENGINE_PID=$!
        cd /app
    }
    ...
    while true; do
        ...
        if ! kill -0 "$SERVER_PID" 2>/dev/null; then
            echo "[outpost] Server exited, shutting down..."
            cleanup
        fi

        if ! kill -0 "$ENGINE_PID" 2>/dev/null; then
            echo "[outpost] Engine crashed, restarting in 3 seconds..."
