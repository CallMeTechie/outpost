...
// The ceiling holds for both kinds of wait: the one Microsoft asks for and the one we compute.
// FileTransfer's own stall watchdog gives a full pipeline 600 s, so staying well under that is what
// keeps a legitimate backoff from being mistaken for a wedged destination.
...
const createGraphClient = ({ getAccessToken: loadToken, forgetToken, fetchImpl, sleep, random = Math.random }) => {
    const request = async (connectionId, {
        ...
        for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
            ...
            // A wait longer than the ceiling is not a wait, it is a refusal with extra steps.
            if (retryAfter !== null && retryAfter * 1000 > MAX_WAIT_MS) throw fail();

            await sleep(retryAfter !== null ? retryAfter * 1000 : backoffDelay(attempt, random));
        }
