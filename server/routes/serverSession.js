...
app.post("/", async (req, res) => {
    ...
    try {
        const { entryId, identityId, connectionReason, type, directIdentity, tabId, browserId, scriptId, startPath, tmuxSession, tmuxCreate } = req.body;
        const ipAddress = req.ip || req.socket?.remoteAddress || 'unknown';
        const userAgent = req.headers['user-agent'] || 'unknown';
        const result = await createSession(req.user.id, entryId, identityId, connectionReason, type, directIdentity, tabId, browserId, scriptId, startPath, ipAddress, userAgent, tmuxSession, tmuxCreate);
